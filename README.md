...
- **Камера**: 640x480 @ 15 FPS
- **Микрофон**: Дуплексное аудио через WiFi
- **Экран**: ThorVG аватар рендеринг
- **Связь**: BLE 5.x → IP контроллер → сервер (в разработке)
- **Gaze**: локальная детекция взгляда + анимация глаз
...
**Локальная детекция лица (device-side)**
- **Модель**: BlazeFace (на устройстве)
- **Назначение**: быстрый ROI лица для backend распознавания
- **Выход**: bbox + confidence + face crop (JPEG)
- **Частота**: 10-15 FPS
...
### 7.1 Gaze Detection и Gaze Animation

**Gaze detection (device-side)**
- **Источник**: локальный face mesh и глазные ориентиры
- **Частота**: 5 FPS детекция + 30 FPS интерполяция
- **Выход**: gaze vector (x, y) + blink state

**Gaze animation (render)**
- **Рендер**: ThorVG, плавное сглаживание
- **Синхронизация**: работает параллельно со speech pipeline
- **Назначение**: мгновенная реакция аватара без ожидания backend

### 8. Коррекция и синхронизация

**Streaming коррекция**
- **Процесс**: TTS начинается с шаблона, корректируется по мере генерации Qwen30bVL
...
**WowCube → Сервер:**
- **WiFi/RTC**: 1-2 Mbps (аудио + face кадры)
...
Данная архитектура обеспечивает создание MVP системы AI Live Pod с мгновенными ответами (<50мс) и высококачественными эмоциональными диалогами. Использование Qwen30bVL через OpenRouter или аналог позволяет получить enterprise-качество генерации без необходимости содержать собственную инфраструктуру для больших языковых моделей.
...
flowchart TD
    ...
    A2 -->|Да| B[Локальная детекция лица (BlazeFace)]
    
    A3 --> A4[Периодическая проверка активности]
    A4 --> A1
    
    B --> GZ[Локальная gaze детекция и анимация]
    B --> B1[Отправка face ROI на backend]
    B1 --> C{Пользователь найден в базе?}
    ...
    V1 --> V3[Qwen30bVL: Полная генерация в фоне]
